...
class TestAdmins(TestCase):
    ...
    def test_duplication(self):
        self.assertRaises(
            IntegrityError,
            lambda: AdminProfile.objects.create(
    ...
    def test_location(self):
        # To test if city(location) name does not allow numbers or special
        # characters
        profile = AdminProfile(
            user=User.objects.create_user(
                ...
                last_name='Lahori',
                email='jlahori92@gmail.com',
                username='jlahori_test2',
                password='password'),
            address='IIIT-H',
            location='Hyderabad123!@#',
            state='Andhra Pradesh',
            organization=Organization.objects.get(
                name='LinkedIn'),
            phone='9581845730')
        with self.assertRaises(ValidationError):
            if profile.full_clean():
                profile.save()
        self.assertEqual(
            0, AdminProfile.objects.filter(
                user__username='jlahori_test2').count())
    ...
    def test_organization_foreign_key(self):
        # To test only registered organizations should be allowed as valid
        # entries
...
class TestEvent(TestCase):
    ...
    def test_startTime(self):
        # To test if startTime format follows YYYY-MM-DD HH:MM:SS does not 0 as
        # valid entry
        event = Event(
            eventName='event1_test',
            startDate='2015-05-05 05-05-05',
            endDate='2015-05-05 05:05:05')
        with self.assertRaises(ValidationError):
            if event.full_clean():
                event.save()
        self.assertEqual(
            0, Event.objects.filter(
                eventName='event1_test').count())

    def test_endTime(self):
        # To test if endTime format follows YYYY-MM-DD HH:MM:SS does not 0 as
        # valid entry
        event = Event(
            eventName='event1_test',
            startDate='2015-05-05 05:05:05',
            endDate='2015-05-05 25:05:05')
        with self.assertRaises(ValidationError):
            if event.full_clean():
                event.save()
        self.assertEqual(
...
class TestRegisterView(TestCase):
    ...
    def test_invalid_value(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual(
            response.context['userForm']['email'].errors,
            ['Enter a valid email address.'])
        self.assertEqual(
            response.context['volunteerProfileForm']['organization'].errors,
            ['Select a valid choice. That choice is not one of the available choices.'])
        self.assertEqual(
            response.context['volunteerProfileForm']['phone'].errors,
            ['Enter a valid value.'])
    ...
    def test_null_value(self):
        ...
        self.assertEqual(
            ...
            ["This field is required."])
        self.assertEqual(
            response.context['volunteerProfileForm']['address'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['volunteerProfileForm']['location'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['volunteerProfileForm']['state'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['firstname'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['lastname'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['username'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['email'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['password'].errors,
            ["This field is required."])
        self.assertEqual(
            response.context['userForm']['password2'].errors,
            ["This field is required."])
...
class TestEventView(TestCase):
    ...
    def test_volunteer_access(self):
        ...
        profile = VolunteerProfile.objects.create(
            ...
            phone='9581845730')
        self.assertEqual(
            1, VolunteerProfile.objects.filter(
                user__username='jlahori_volunteer').count())
    ...
    def test_invalid_value(self):
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual(
            response.context['eventForm']['eventName'].errors,
            ['Event with this EventName already exists.'])
        self.assertEqual(
            response.context['eventForm']['startDate'].errors,
            ['Enter a valid date/time.'])
        self.assertEqual(
            response.context['eventForm']['endDate'].errors,
            ['Enter a valid date/time.'])
...
class TestJobView(TestCase):
    ...
    def test_invalid_value(self):
        ...
        response = c.post('/AdminUnit/job/',
                          {'event': 'test_event_2',
                           ...
                           'jobDescription': 'test_jobDescription',
                           'startDate': '2014-05-05 05-05-05',
                           'endDate': '05-10-2014 05:05:05'})

        self.assertEqual(200, response.status_code)
        self.assertEqual(
            response.context['jobsForm']['event'].errors,
            ['Select a valid choice. That choice is not one of the available choices.'])
        self.assertEqual(
            response.context['jobsForm']['startDate'].errors,
            ['Enter a valid date/time.'])
        self.assertEqual(
            response.context['jobsForm']['endDate'].errors,
            ['Enter a valid date/time.'])
    ...
    def test_null_value(self):
        ...
        self.assertEqual(
            ...
            ['This field is required.'])
        self.assertEqual(
            response.context['jobsForm']['jobName'].errors,
            ['This field is required.'])
        self.assertEqual(
            response.context['jobsForm']['jobDescription'].errors,
            ['This field is required.'])
        self.assertEqual(
            response.context['jobsForm']['startDate'].errors,
            ['This field is required.'])
    ...
    def test_duplicate_job_in_same_event(self):
        ...
        response = c.post('/AdminUnit/job/',
                          {'event': event.id,
                           'jobName': 'test_jobName',
                           'jobDescription': 'test_jobDescription_2',
                           'startDate': '2014-05-05 05:05:05',
                           'endDate': '2014-05-05 05:05:05',
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual(
            len(response.context['jobsForm'].non_field_errors()), 1)
        self.assertEqual(
